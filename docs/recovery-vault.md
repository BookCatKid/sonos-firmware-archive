...
The vault currently retains verified keys for models 8, 9, 12, 16, and 17,
...
That command verifies every inventoried SHA-256 hash, all five public-key
...
- `scripts/find_legacy_system_word.py` reproduces the bounded search that
  discovered model 12's parameter;
...
7 updater used in the still-open recovery work, the tested model-5 flash
